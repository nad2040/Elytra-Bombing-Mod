...
import com.nad2040.elytrabombing.IUseItemInFlight;
...
@Mixin(Item.class)
public class ItemMixin implements IUseItemInFlight {
	@Override
	public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
		if (user.isFallFlying()) {
			...
			if (item.isOf(Items.FLINT_AND_STEEL)) {
				...
				if (!tnt.isOf(Items.TNT)) return TypedActionResult.pass(user.getStackInHand(hand));
				if (!world.isClient) {
				...
				if (!user.getStackInHand(other_hand).isEmpty()) return TypedActionResult.pass(user.getStackInHand(hand));
				if (!world.isClient) {
					position = user.getPos();
					velocity = user.getVelocity();
